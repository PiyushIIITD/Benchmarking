...
// Directly include implementations
...
void run_benchmark(const std::string& name, size_t num_elements, float alpha_val = 0.01f) {
    ...
    CUDA_CHECK(cudaMalloc(&d_out, num_elements * sizeof(float)));
    ...
    double cpu_time_ms = measureCPUTime([&]() {
    ...
    for (int i = 0; i < WARMUP_RUNS; ++i) {
        ...
        else if (name == "Softmax") softmax_cuda(d_in, d_out, num_elements);
